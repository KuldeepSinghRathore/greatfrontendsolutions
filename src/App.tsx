...
const App = () => {
    ...
    const initialSolvedQuestions: number[] = storedSolvedQuestions ? JSON.parse(storedSolvedQuestions) : [];

    const [solvedQuestions, setSolvedQuestions] = useState<number[]>(initialSolvedQuestions);
    ...
    useEffect(() => {
        if (selectedCategory === 'All') {
            ...
            const newData = q_data.filter((item) => item.category === selectedCategory);
        ...
        <div className='w-full max-w-6xl mx-auto p-4  space-y-4'>
            ...
            <div className='mb-4 p-4 border bg-gray-100/10 border-zinc-300 dark:border-zinc-800 rounded-md font-medium'>
                ...
                <ul className='list-disc pl-4 pb-2'>
                    <li className=''>
                        <b>Sharpen JavaScript Skills</b>: Enhance your proficiency in JavaScript programming.
                    </li>
                    <li>
                        <b>Boost Problem-Solving</b>: Tackle diverse challenges for creative solutions.
                    </li>
                    <li>
                        <b>Interview Prep</b>: Prepare for technical discussions with potential employers.
                    </li>
                </ul>
            </div>
            <div className='mb-4 p-4 bg-gray-100/10  border border-zinc-300 dark:border-zinc-800 rounded-md'>
                <p className='text-xs font-medium pb-1'>
                    These questions were originally curated from a GitHub repository created by{' '}
                    <a href='https://github.com/ghoshsuman845' className='underline text-blue-600'>
                        Suman Ghosh
                    </a>
                    . I have compiled and categorized them on this website for convenience. This website is open source!
                    You can find the codebase and contribute on my{' '}
                    <a
            ...
            <div className='flex flex-col w-full'>
                ...
                <div className='w-full flex items-center justify-end'>
                    <span className='text-md text-green-500'>{solvedQuestions.length}</span>/
                ...
                <Accordion type='single' collapsible>
                    {filteredData.map((item, index) => (
                        ...
                        >
                            <div className='flex w-full items-center justify-between'>
                                <AccordionTrigger onClick={() => setShowCode(false)}>{item.name}</AccordionTrigger>
                                    ...
                                    <Checkbox
                                        ...
                                        onCheckedChange={(value) => {
                                            if (value) {
                                                ...
                                                localStorage.setItem(
                                                    'solvedQuestions',
                                                    JSON.stringify(solvedQuestions.filter((item) => item !== index))
                                                );
                                            }
                                        }}
                                    />
                            ...
                            <AccordionContent className='border border-blue-500 p-3 rounded-lg'>
                                <Markdown
                                    components={{
                                        ...
                                        h3: ({ ...props }: any) => <h3 className='text-md font-bold' {...props} />,
                                        h4: ({ ...props }: any) => <h4 className='text-sm font-bold' {...props} />,
                                        h5: ({ ...props }: any) => <h5 className='text-xs font-bold' {...props} />,
                                        ...
                                        li: ({ ...props }: any) => <li className='text-sm leading-7' {...props} />,
                                        ul: ({ ...props }: any) => <ul className='text-sm leading-7' {...props} />,
                                        ol: ({ ...props }: any) => <ol className='text-sm leading-7' {...props} />,
                                    }}
                                    children={atob(item.md.content).split('Solution')[0]}
                                />
                                <Button variant={'link'} size={'sm'} onClick={() => setShowCode((prev) => !prev)}>
            ...
            >
                Give a <StarIcon className='text-yellow-500 mx-1' /> to this repo on GitHub
            </a>
        </div>
    );
};
